...
import { wrapPrivateKey, type PrivateKey } from "./private-key.js";
...
export function createPrivateKeyring(
  ...
  return new (class PrivateKeyring implements IKeyring {
    keys: PrivateKey[] = [];
    ...
    constructor() {
      this.keys = _keyring.keys.map((key: Uint8Array) => wrapPrivateKey(key));
    }
    ...
    unwrapAsymmetric(privateKeyArg: PrivateKey, params?: { padding?: boolean }) {
      return createPrivateKeyring(
        _keyring.unwrapAsymmetric(privateKeyArg, params).wrappedValue,
      );
    }
    ...
    decrypt(ciphertext: Uint8Array, params?: { padding?: boolean }): Uint8Array {
      if (_keyring.topLayer !== DataLayer.Raw) {
