...
| **Tests** | Vitest 503 matrix includes the three new routes (**68** total); no live Stripe calls in CI yet. |
...
### Not started (Phase 3 — realtime / collab only)

Sprints **1–9** (pages / groups / membership) and **Stripe** are tracked in the sections above. Remaining work:

- [ ] **Realtime / collab** — new or adapted WebSocket protocols; **no** key rotation; Durable Object vs separate service per [RESTART_PLAN](../docs/RESTART_PLAN.md) §4.3 / hosting table; blocks editor MVP collab in Phase 4.
...
| **1** | **Realtime** (JWT upgrade, msgpackr-style protocol) + **collab** (Yjs, no rotation) | [RESTART_PLAN §4.3](../docs/RESTART_PLAN.md); load-test before freeze; only major Phase 3 item left. |
