...
| **4** — Client MVP | **In progress** | **Shipped:** [OpenAPI client](#phase-4--openapi-typed-client-bootstrap) + [routing + session UI](#phase-4--routing--session-ui) + [pages + editor + register](#phase-4--pages-list-editor--register) + **[collab decrypt + REST append](#phase-4--collab-decrypt--yjs--post-append)** (`@deepnotes/e2ee`, `passwordSalt`, extended collab `GET`). **Next:** [real register / demo crypto parity](#phase-4--register-crypto-parity) (current `/register` still uses random key-shaped bytes); Tiptap; **MSW/Playwright**; **groups** UX. **Phase 3** still: [collab + realtime WebSocket](#not-started-phase-3--realtime--collab-websocket). |
...
**Limits (documented in UI):** **Demo** accounts cannot derive client keys (no `passwordSalt`). **`/register`** still ships random ciphertext-shaped fields ([build-user-register](apps/web/src/features/auth/build-user-register.ts)) — decrypt will not match real pages until [register crypto parity](#phase-4--register-crypto-parity) lands. **Group password** / non-raw content keyrings are rejected with a clear message.

### Phase 4 — register crypto parity (not done)

**Goal:** Replace random `buildSessionDemoRequest`/`buildUserRegisterRequest` blobs with **`generateRandomUserKeys`–style** client crypto (libsodium keyrings + `UserPrivateKeyring` / `UserSymmetricKeyring` + group/page creation matching [legacy register](apps/client/src/code/areas/auth/register.ts)) so new accounts can use the editor E2EE path end-to-end.

| Step | Status |
|------|--------|
| Client: derive + wrap user keyrings with password; real `groupCreation` + `pageCreation` | **Todo** |
...
| **`@deepnotes/web`** | SPA | `app.test.ts` (router + `App`, bootstrap); **`client.test.ts`**; **`bytes.test.ts`**; **`build-user-register.test.ts`**; Vitest [include](apps/web/vite.config.ts) `src/**/*.test.ts` | MSW/contract for login + 2FA; **`page-collab-crypto`** unit tests (optional); `generate:api-types` when OpenAPI changes; Playwright (see Phase 4 checklist) |
