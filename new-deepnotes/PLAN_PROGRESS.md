...
| Layer | What shipped |
|-------|----------------|
| **`@deepnotes/session`** | [page-collab-updates.ts](packages/session/src/page-collab-updates.ts) — `performGetPageCollabUpdates` (`viewGroupPages`, ordered by `index`); `performAppendPageCollabUpdates` (`editGroupPages`, transaction + `max(index)` check, contiguous indices, **409** on stale `expectedLastIndex`) |
