...
| **4** — Client MVP | **In progress** | **Shipped:** [OpenAPI client](#phase-4--openapi-typed-client-bootstrap) + [routing + session UI](#phase-4--routing--session-ui) + [pages + editor + register](#phase-4--pages-list-editor--register) + **[collab decrypt + REST append](#phase-4--collab-decrypt--yjs--post-append)** + **[Tiptap + Yjs (SPA)](#phase-4--tiptap--yjs-spa)** + **[register crypto parity](#phase-4--register-crypto-parity)** + **[groups overview UI](#phase-4--groups-overview-thin-spa)** + **[notifications list + mark read](#phase-4--notifications-thin-spa)** + **[group members / invites admin UI](#phase-4--group-members--invites-admin-ui)** + **[group invite/join E2EE](#phase-4--group-invitejoin-e2ee-spa)** (send invite, accept invite landing, request join, accept join request with crypto) + **[MSW contract tests](#phase-4--msw-api-contract-tests)** + **ESLint** [`no-restricted-imports`](apps/web/eslint.config.js) on `src/**/*.ts` (server/db/session/drizzle). **Next:** **Playwright** (or **MSW**-heavy) E2E smoke; optional deterministic demo keyring for CI; optional notification **body** decrypt. **Phase 3** still: [collab + realtime WebSocket](#not-started-phase-3--realtime--collab-websocket). |
...
| **Membership + joins (slice 9)** | Two Pro users; shared **public** group via `groupCreation`; [performGroupJoinInvitationSend](packages/session/src/group-membership.ts) → invite row → [performGroupJoinInvitationAccept](packages/session/src/group-membership.ts); [performGroupMemberRoleChange](packages/session/src/group-membership.ts) to moderator; [performGroupMemberRemove](packages/session/src/group-membership.ts); [performGroupJoinRequestSend](packages/session/src/group-membership.ts) + [performGroupJoinRequestAccept](packages/session/src/group-membership.ts) with `viewer` | Covers legacy WS join-invitation / join-request / change-role / remove DB semantics; **no** push-notification step 2 (client-driven later). **`performGetGroupMembersDetail`:** after invite, owner sees `pendingInvitations`; invitee gets **403** on detail until accept; after accept, both see `members` / `viewerRole`; after join-request send, owner sees `pendingJoinRequests`. |
...
| **Tests** | [client.test.ts](apps/web/src/api/client.test.ts) — mocked `fetch` asserts `Request.credentials === "include"` and `/api/health` URL. [MSW contract tests](#phase-4--msw-api-contract-tests). |
| **Lint** | [eslint.config.js](apps/web/eslint.config.js) — ignores generated OpenAPI outputs + shadcn UI; **`no-restricted-imports`** on `src/**/*.ts` blocks `@deepnotes/api-worker`, `@deepnotes/db`, `@deepnotes/api`, `@deepnotes/session`, and `drizzle-orm` (`*` subpaths). **Gap:** `.vue` SFCs are not in the `eslint` file glob today (add `eslint-plugin-vue` to enforce the same in `<script>` if needed). |

**Intentional gaps:** Playwright (or broad MSW) for full auth redirect / cookie E2E; optional OpenAPI JSON drift check in CI.

### Phase 4 — MSW API contract tests

**Goal:** RESTART_PLAN §5.8 “API / contract” layer with **MSW** intercepting the same URLs `openapi-fetch` would hit in production, without a running Worker.

| Layer | What shipped |
|-------|----------------|
| **Tooling** | `msw@^2` in [apps/web/package.json](apps/web/package.json); Node entry [`setupServer`](https://mswjs.io/docs/api/setup-server) from `msw/node`. |
| **Handlers** | [deepnotes-handlers.ts](apps/web/src/test/msw/deepnotes-handlers.ts) — `contractApiBaseUrl` = `https://msw.contract.test`; `GET` `/api/health` → `HealthResponse`; `GET` `/api/users/me` → `UserMeResponse`. |
| **Server** | [test/msw/node.ts](apps/web/src/test/msw/node.ts) exports `mswServer` for Vitest. |
| **Tests** | [client.contract.test.ts](apps/web/src/api/client.contract.test.ts) — `beforeAll`/`afterAll` listen/close, `afterEach` `resetHandlers`; success paths + **`server.use`** 401 for `SessionErrorResponse` on `/api/users/me`; `onUnhandledRequest: "error"`. |

**Next:** optional MSW for `POST /api/sessions/*` to exercise [useSession](apps/web/src/features/auth/useSession.ts) without Playwright; **Playwright** still preferred for real httpOnly cookie flows.
...
**Intentional gap:** [Phase 3 WS collab](#not-started-phase-3--realtime--collab-websocket); **Playwright** E2E (MSW contract tests [shipped](#phase-4--msw-api-contract-tests)).
...
**Gaps:** covered by [group members admin](#phase-4--group-members--invites-admin-ui) for list + manager actions; **send invite / accept invite / request join / accept request** still need browser E2EE (same shapes as [slice 9](#pagesgroups-rest--slice-9-membership--join-invites--requests) bodies).

### Phase 4 — group members & invites (admin UI)

**Goal:** Phase 4 “groups subset” continuation — authenticated screen to inspect membership with **roles** and run **non-crypto** manager flows, backed by a greenfield **`GET /api/groups/{groupId}/members/detail`** (OpenAPI `GroupMembersDetailResponse`).

| Layer | What shipped |
|-------|----------------|
| **`@deepnotes/session`** | [`performGetGroupMembersDetail`](packages/session/src/group-main-and-members.ts) — same gate as `performGetGroupMemberUserIds` (`viewGroupMembers` + membership row); returns `viewerUserId`, `viewerRole`, `groupIsPublic`, `joinRequestsAllowed`, `members[]`, `pendingInvitations[]`, `pendingJoinRequests[]` (non-rejected only). |
| **`@deepnotes/api`** | [`groupMembersDetailResponseSchema`](packages/api/src/schemas/pages-groups.ts); OpenAPI `GET /api/groups/{groupId}/members/detail`; [TRPC_REST_MAP](docs/TRPC_REST_MAP.md) greenfield row. |
| **`@deepnotes/api-worker`** | Hono `GET` handler; **503** matrix extended to **74** with [invite E2EE bootstrap](#phase-4--group-invitejoin-e2ee-spa) routes. |
| **Tests** | [account-flows.integration.test.ts](packages/session/src/account-flows.integration.test.ts) extends [slice 9](#pagesgroups-rest--slice-9-membership--join-invites--requests) case with detail assertions (+ invitee **403** before accept). |
| **Web** | Route [`/groups/:groupId`](apps/web/src/router.ts) → [`GroupDetailView.vue`](apps/web/src/features/groups/GroupDetailView.vue); [`group-members-detail.ts`](apps/web/src/features/groups/group-members-detail.ts) fetch; [`group-role-policy.ts`](apps/web/src/features/groups/group-role-policy.ts) (mirrors session ranks); [`useGroupMembersDetail.ts`](apps/web/src/features/groups/useGroupMembersDetail.ts) — load, leave (`DELETE …/members/me`), remove, `PATCH` role, `DELETE` cancel invite, `POST` reject invite / reject join request; [GroupsView](apps/web/src/features/groups/GroupsView.vue) “Members & invites” link; tests [`group-members-detail.test.ts`](apps/web/src/features/groups/group-members-detail.test.ts), [`group-role-policy.test.ts`](apps/web/src/features/groups/group-role-policy.test.ts). Regenerate [`openapi.json`](apps/web/src/api/openapi.json) / [`api-types.generated.ts`](apps/web/src/api/api-types.generated.ts) after API changes. |

### Phase 4 — group invite/join E2EE (SPA)

**Goal:** Browser builds the same ciphertext shapes as legacy WebSocket slice-9 flows ([`send.ts`](../../apps/client/src/code/areas/api-interface/groups/join-invitations/send.ts), [`accept.ts`](../../apps/client/src/code/areas/api-interface/groups/join-invitations/accept.ts), [`join-requests/send.ts`](../../apps/client/src/code/areas/api-interface/groups/join-requests/send.ts)), using `@deepnotes/e2ee` + session keyrings (password login, not demo).

| Layer | What shipped |
|-------|----------------|
| **`@deepnotes/session`** | [`performGetGroupInviteCryptoBootstrap`](packages/session/src/group-invite-crypto-bootstrap.ts) — managers only (`roleHasManageLowerRanks`); returns `groupPublicKeyring`, `groupAccessKeyring`, `memberEncryptedAccessKeyring`, `memberEncryptedInternalKeyring`. [`performGetGroupPublicKeyringForMessaging`](packages/session/src/group-invite-crypto-bootstrap.ts) — member, pending invitee, or join-request-eligible non-member. [`performGetUserPublicKeyring`](packages/session/src/user-public-keyring.ts) — `users.public_keyring` for any authenticated user (replaces KeyDB read). |
| **`@deepnotes/api`** | `groupInviteCryptoBootstrapResponseSchema`, `groupPublicKeyringResponseSchema`; `userIdPathSchema`, `userPublicKeyringResponseSchema`; OpenAPI `GET …/invite-crypto-bootstrap`, `GET …/public-keyring`, `GET /api/users/{userId}/public-keyring`. |
| **`@deepnotes/api-worker`** | Hono handlers; JSON base64 fields; **503** matrix **74** routes. |
| **Web** | [`group-membership-crypto.ts`](apps/web/src/features/groups/group-membership-crypto.ts) — `buildJoinInvitationSendBodies`, `buildJoinInvitationAcceptBody`, `buildJoinRequestSendBodies`, `buildJoinRequestAcceptBodies`. [`useGroupMembersDetail`](apps/web/src/features/groups/useGroupMembersDetail.ts) — `sendJoinInvitation`, `acceptJoinRequestWithCrypto`. [`GroupDetailView.vue`](apps/web/src/features/groups/GroupDetailView.vue) — invite form + join-request accept (role + Accept). Routes [`/groups/:groupId/invite`](apps/web/src/features/groups/GroupInviteLandingView.vue) (accept/reject invite; invitees cannot load `…/members/detail` before join) and [`/groups/:groupId/join`](apps/web/src/features/groups/GroupJoinRequestView.vue) (encrypted join request). Regenerated [`openapi.json`](apps/web/src/api/openapi.json) / [`api-types.generated.ts`](apps/web/src/api/api-types.generated.ts). |
| **Docs** | [TRPC_REST_MAP.md](./docs/TRPC_REST_MAP.md) greenfield rows. |

**Limits:** Invitee user id must be known out-of-band (no user search). Group password–locked keyrings still fail with a clear client error.
...
- [x] **MSW contract tests (bootstrap):** [Phase 4 — MSW API contract tests](#phase-4--msw-api-contract-tests) — `GET /api/health`, `GET /api/users/me` (200 + 401).
- [x] **ESLint import boundaries (bootstrap):** `no-restricted-imports` in [apps/web/eslint.config.js](apps/web/eslint.config.js) for `src/**/*.ts`.
...
- [x] **API surface:** `src/api/` — generated `paths` + `createDeepnotesApiClient`; bundle does not depend on `@deepnotes/api` at runtime (codegen devDeps only). **ESLint** [`no-restricted-imports`](apps/web/eslint.config.js) on `src/**/*.ts` (see [Phase 4 — OpenAPI typed client](#phase-4--openapi-typed-client-bootstrap)). Optional: `eslint-plugin-vue` for the same in SFCs.
...
- [x] **Composable tests (bootstrap):** [`bytes.test.ts`](./apps/web/src/features/auth/bytes.test.ts) (base64 + UTF-8 preimage); [`app.test.ts`](./apps/web/src/app.test.ts) (router + bootstrap, no `loggedIn` cookie). **Not done:** `LoginView` / `useSession` full matrix (401, 2FA, demo) — partial coverage via [MSW `client.contract.test`](#phase-4--msw-api-contract-tests) for `/api/users/me` 401.
- [x] **Contract tests (bootstrap):** [client.contract.test.ts](apps/web/src/api/client.contract.test.ts) + [MSW handlers](apps/web/src/test/msw/deepnotes-handlers.ts) for `GET /api/health` and `GET /api/users/me` (200 + 401). **Not done:** MSW for login/refresh; recorded OpenAPI fixture drift check in CI.
...
| **`@deepnotes/api-worker`** | Hono on Worker | `index.test.ts`: **74** tests (503 matrix when env/Hyperdrive missing) — adds `GET …/members/detail`, `GET …/invite-crypto-bootstrap`, `GET …/public-keyring`, `GET /api/users/{userId}/public-keyring`; includes [slice 6](#pages-rest--slice-6-bump-backlinks-snapshots-deletion) + `/api/pages/{pageId}/move` + [slice 9](#pagesgroups-rest--slice-9-membership--join-invites--requests) + [slice 10 `…/collab-updates`](#pages-rest--slice-10-collab-updates-rest) (`GET` + `POST`) + [Stripe routes](#phase-3--stripe-billing--webhooks--account-hooks) (`/api/billing/stripe/*`, `/api/webhooks/stripe`) | **200** tests with stub `SessionEnv` + template DB (heavier) |
| **`@deepnotes/web`** | SPA | `app.test.ts` (router + `App`, bootstrap); **`client.test.ts`**; **`client.contract.test.ts`** ([MSW](#phase-4--msw-api-contract-tests)); **`bytes.test.ts`**; **`build-user-register.test.ts`**; **`groups-overview.test.ts`**; **`notifications-list.test.ts`**; **`group-role-policy.test.ts`**; **`group-members-detail.test.ts`**; Vitest [include](apps/web/vite.config.ts) `src/**/*.test.ts` | MSW for `POST /api/sessions/login` + 2FA; **`page-collab-crypto`** unit tests (optional); `generate:api-types` when OpenAPI changes; Playwright (see Phase 4 checklist) |
...
- [x] Web: enforce **no** server/db/drizzle/session package imports from `src/**/*.ts` ([`no-restricted-imports`](apps/web/eslint.config.js); `.vue` scripts optional with `eslint-plugin-vue`); **E2E** smoke for session cookies (RESTART_PLAN §8) still **open** (Playwright).
...
| Date | Change |
|------|--------|
| 2026-04-27 | **Phase 4 — MSW contract tests + ESLint `no-restricted-imports`:** `msw@^2`; [src/test/msw/deepnotes-handlers.ts](apps/web/src/test/msw/deepnotes-handlers.ts), [node.ts](apps/web/src/test/msw/node.ts), [client.contract.test.ts](apps/web/src/api/client.contract.test.ts) (`GET /api/health`, `GET /api/users/me` 200 + 401). [eslint.config.js](apps/web/eslint.config.js) blocks `@deepnotes/api-worker` / `db` / `api` / `session` and `drizzle-orm*`. [MSW section](#phase-4--msw-api-contract-tests) + testing matrix. **Next:** Playwright E2E. |
