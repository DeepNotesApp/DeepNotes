...
| Area | State |
|------|--------|
| **API / REST** | TRPC_REST_MAP HTTP rows largely done; **collab WS** MVP (**Durable Object** relay + Postgres append, legacy lib0 framing via `@deepnotes/collab-wire`); **realtime** **`GET /api/realtime-ws`** + **`UserRealtimeRoom` DO** + `@deepnotes/realtime-wire`. Framing + Vitest round-trips; **`USER_NOTIFICATION`** push after `performNotifyUsers`. **Hash slice:** when **Upstash** (`UPSTASH_REDIS_*`) **and** **`HYPERDRIVE`** are set, DO serves **`user:`** / **`page:`** / **`group:`** Redis hashes with Postgres ACL (`resolveRealtimeHashFieldAccess`: read=`viewGroupPages`, write=`editGroupPages`). With Upstash only, **`user:{id}`** still works (sync user-only gate). **Cross-process hash notifications:** Upstash **PUBLISH** on `data-update|{fullKey}` after **HSET** + **SSE SUBSCRIBE** bridge per subscribed field (legacy-shaped **msgpack** payload + 16-byte origin id). **Cache TTL:** Redis **`EXPIRE`** on the hash key (7d, legacy `DEFAULT_REMOTE_TTL` duration) after reads/writes — substitutes KeyDB **`expiremember`** per field. **Local-cache pub/sub:** documented parity stance — DO + Redis, no in-process LRU ([LOCAL_CACHE_PARITY.md](./docs/LOCAL_CACHE_PARITY.md)). |
| **SPA** | Auth, lists, **`PageEditorView`** (Tiptap+Y+collab **WebSocket when configured** else REST, **encrypted awareness + collaboration carets** on WS, link/underline/placeholder, **tables, images, tasks, code (lowlight), KaTeX math (inline + block), YouTube embeds (Vue node view + resize handle like legacy)**, highlight, align, sub/sup, HR, path breadcrumb (**unknown segment label `[Page ${id}]`** like legacy `pageAbsoluteTitles`; decrypted titles via realtime when WS + ACL + **same-group**), bump/favorite/recent, **snapshots list/save/restore/delete**, **set-as-main + soft-delete + purge**, **cross-group move/reencrypt**), groups/**members**/invite/join + **group settings** (join policy, soft-delete, **make public / make private**, **group purge**), notifications list **with decrypt**, **`/account`**, home **recents/favorites/starting/defaults** UIs, theme, **live notification toast** when **`/api/realtime-ws`** connected, **realtime `page:` path titles** (legacy **REQUEST** + **SUBSCRIBE** when WS + ACL; decrypted absolute titles for **same-group** path segments). **Spatial/world:** **`/spatial`** stub (placeholder canvas grid + legacy context copy); home spatial-defaults links to stub; nav **Spatial**. Missing: legacy-style **interactive** infinite canvas + spatial containers in-editor, native shells. |

**Deferred (confirm vs parity):** optional Stripe **HTTP** route IT with mocked Hyperdrive (session layer has **`stripe-billing.test.ts`** — parse + subscription handlers without secrets). **Infra naming:** Workers/DO replaces standalone collab/realtime/scheduler processes.
...
| 2026-04-29 | **`groups.getPages` optional auth:** `tryGetAuthenticatedUserSummary` + `performListGroupPages` anon access for **public** groups (`access_keyring`); legacy `optionalAuthProcedure` parity; integration test; OpenAPI + TRPC_REST_MAP. |
